...
extension DetectBrand on String {
  ...
  BrandType get detectBrand {
    ...
    bool isMADA = _madaRegExpM.hasMatch(cleanNumber) ||
        _madaRegExpV.hasMatch(cleanNumber);
    bool isVISA = _visaRegExp.hasMatch(cleanNumber);
    bool isMASTERCARD = _mastercardRegExp.hasMatch(cleanNumber);
...
extension BrandTypeExtension on BrandType {
  ...
  String? entityID(HyperpayConfig config) {
    ...
    switch (this) {
      case BrandType.visa:
        entityID = config.creditcardEntityID;
        break;
      case BrandType.master:
        entityID = config.creditcardEntityID;
        break;
      case BrandType.mada:
        entityID = config.madaEntityID;
        break;
